...
app.get('/todo',
  ...
  async (req,res,next) => {
    try{
      const {title,description,time} = req.body; // get title and description from the body
      ...
      const hide = false; 
      const calendar = (month<10?("0"+month.toString()):(month.toString()))+'/'+(date<10?("0"+date.toString()):(date.toString()))+'/'+year.toString();
      //res.json(date)
      let data = {title, description, userId, createdAt, time, calendar, hide} // create the data object
  ...
  app.get("/todo/edit/:itemId",
  isLoggedIn,
  async (req,res,next) => {
    try{
      const itemId=req.params.itemId; // get the id of the item to delete
      let item = await ToDoItem.find({_id:itemId});
      res.locals.time = item.time;
      res.locals.title = item.title;
      res.locals.description = item.description;
      await ToDoItem.deleteOne({_id:itemId}) // remove that item from the database
      res.render("editItem") // go back to the todo page
    } catch (e){
      next(e);
    }
  }
  ...
  app.get("/todo/refresh",
  isLoggedIn,
  async (req,res,next) => {
    try{
      let userId = res.locals.user._id; 
      let items = await ToDoItem.find({userId:userId});
      res.locals.items = items;
      const hide = true;
      for (item of items){ 
        
        const {title, description, userId, createdAt, time, calendar, hide}=item;
        item.hide = true;
        await ToDoItem.findOneAndUpdate({title, description, userId, createdAt, time, calendar, hide},item,{ $set: { "hide": hide } })
        //item.updateOne({ $set: { "hide": hide } });
      }
      res.render("toDo") // go back to the todo page
    } catch (e){
      next(e);
    }
  }
...
app.post('/todo/date',
  ...
  async (req,res,next) => {
    try{
      ...
      let userId = res.locals.user._id;  // get the user's id     
      let items = await ToDoItem.find({userId:userId,calendar:date}); // lookup the user's todo items
      res.locals.items = items;  //make the items available in the view
      //res.locals.calendar = date;
      //res.json(date)
      res.render('itemlist');  // render to the toDo page
    } catch (e){
      next(e);
    }
  }
